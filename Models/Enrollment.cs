...
public class Enrollment : NotifyPropertyChangedBase
{
    ...
    /// <summary>
    /// 변동 일자. 입학·전입이면 학적이 시작된 날, 전출·졸업이면 끝난 날이다.
    ///
    /// <para>전출 뒤에 그 학생 기록을 남기려 하면 이 날짜를 기준으로 경고한다.</para>
    ///
    /// <para><b><c>null</c> 은 "날짜를 모른다" 는 진짜 상태다</b> — 컬럼도 NULL 을 받는다.
    /// 그래서 <c>DateTime</c> 이 아니라 <c>DateTime?</c> 다. <c>EnrollmentGuard</c> 는
    /// 날짜가 없으면 <b>경고하지 않는다</b>(기준이 없으므로). <c>DateTime.MinValue</c> 같은
    /// 특수값을 쓰면 그 학생이 아주 오래전에 떠난 것으로 잡혀 늘 경고가 뜬다.</para>
    ///
    /// <para>예전에는 <c>"yyyy-MM-dd"</c> 문자열이었다. 읽는 쪽이 모두 날짜로 되돌려
    /// 쓰고 있었고(<c>EnrollmentGuard.ParseDate</c>·<c>StudentEditDialog.ParseDate</c>),
    /// 형식이 어긋난 글자가 들어와도 저장될 때까지 아무도 몰랐다.</para>
    /// </summary>
    public DateTime? ChangeDate
    {
        get => _changeDate;
        set => SetProperty(ref _changeDate, value?.Date);
    }
    ...
    /// </summary>
    public void ApplyChange(string changeType, DateTime? changeDate = null)
    {
        ChangeType = changeType;                       // IsActive 가 따라온다

        // 날짜를 안 주면 옛 값을 지킨다 — "날짜 없음" 으로 덮어쓰지 않는다.
        // (변동 유형만 고치는 호출이 있고, 그때 날짜를 잃으면 경고 기준이 사라진다.)
