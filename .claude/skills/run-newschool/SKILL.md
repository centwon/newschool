...
| `keys -Text <키>` | 키를 보낸다(SendKeys 문법: `{TAB}` `+{TAB}` `{ENTER}` `{ESC}` `^s` `+{F10}`) |
| `focused` | 지금 포커스가 있는 요소. 이름이 없으면 `<<이름없음>>` 이라고 알린다 |
| `tabs -N 25` | **Tab 을 N 번 누르며 포커스가 가는 곳을 적는다 — 이것이 곧 Tab 순서다.** `-Text '+{TAB}'` 이면 거꾸로 |
| `nameless` | UIA 이름이 없는 조작 요소를 센다(`tree` 는 이런 것을 건너뛴다) |
| `quit` | 닫는다 |

### 키보드로 몰 때 (54차에 붙였다)

`tabs` 는 두 가지를 구분해 알린다 — **한 바퀴**(정상)와 **갇힘**(그 자리에서 Tab 이 죽었다).
갇힘이 나오면 그건 키보드만 쓰는 사람에게 막다른 길이라는 뜻이다.

```powershell
Drive @('tabs','-N','30')          # 이 화면의 Tab 순서 전체
Drive @('keys','-Text','+{F10}')   # 컨텍스트 메뉴(메뉴 키). {APPS} 는 SendKeys 에 없다
Drive @('focused')                 # 지금 어디에 포커스가 있나
```

주의할 것:

- **`SendKeys` 는 포그라운드 창에만 간다.** `SetForegroundWindow` 하나로는 안 된다 —
  Windows 가 백그라운드 프로세스의 창 끌어오기를 막아 **조용히 실패하고 키가 엉뚱한 창으로
  간다**(실제로 첫 시도에서 Tab 이 전부 브라우저로 갔다). 드라이버가 UIA `SetFocus()` +
  `AttachThreadInput` 을 함께 쓰고, **끝나고 포그라운드가 정말 우리 창인지 확인한 뒤에만**
  키를 보낸다. 못 끌어오면 던진다 — 그때는 **화면이 잠겨 있는지(`LockApp`) 확인할 것.**
- **`tree` 는 이름도 id 도 없는 요소를 건너뛴다.** 아이콘 전용 단추가 바로 거기 숨으므로,
  접근성을 볼 때는 `tree` 가 아니라 `tabs`·`nameless` 를 쓸 것.
- **⚠ 진짜 데이터가 열려 있다.** `{TAB}` 이 글 편집기 안에서는 **탭 문자를 실제로 넣는다**
  (실측). 확인만 할 때는 저장하지 말고, 미심쩍으면 `quit` 후 다시 띄워 원래대로인지 볼 것 —
  저장하지 않은 것은 DB 에 남지 않는다.
- **`{APPS}` 는 SendKeys 키워드가 아니다.** 메뉴 키는 `+{F10}`(Shift+F10)로 보낼 것.
...
- **~~한글 출력이 콘솔 코드페이지에서 깨져 보인다~~ — 고쳤다(54차).** `driver.ps1` 에
  **UTF-8 BOM** 을 붙여 해결했다. BOM 이 없으면 PowerShell 5.1 이 파일을 ANSI(CP949)로
  읽어 한글이 깨지고, **드물게는 깨진 바이트가 따옴표로 보여 파서 오류까지 난다**
  (실제로 한글 주석 두 줄을 더했더니 `Unexpected token` 이 났다). 이 파일을 고칠 때
  **BOM 을 지우지 말 것.**
