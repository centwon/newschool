...
> 작성: 2026-07-09 · **0~4단계 전부 완료(2026-07-12)** — 아래 계획은 실행이 끝난 기록이다.
> 현재: xUnit 2.9 · 테스트 **616개** (2026-08-30, v1.0.0 게시 기준) / 프로덕션 약 7.5만 줄·XAML 84개
> — 계획 밖에서 늘어난 몫은 대부분 **그때그때 잡은 버그의 회귀망**이다. 특히 시수 계산
>   (`WeeklyHoursCalculatorTests`)과 휴업일 판정(`SchoolCalendarTests`)이 크게 늘었다 —
>   방학 주차 자르기, 방학식은 수업일, 토요휴업일 같은 변형까지 못박아 두었다.
> — 1.0 정리로 사라진 기능(연간계획·자동배치·Undo)의 테스트는 함께 걷어냈고,
>   되살린 시수·진도와 새 시간표 변경의 테스트(`WeeklyHoursCalculatorTests`·`TimetableChangeMergerTests`)가 들어왔다.
> — 줄 수가 8.5만에서 줄어든 것은 2026-08 검사에서 **한 번도 쓰지 않은 코드**를 걷어냈기 때문이다.
>   지운 코드만 검증하던 테스트도 함께 빠졌고(예: `TimetableServiceTests`), 그 자리를 회귀 테스트가 채웠다.
> 착수 시점: 테스트 25개(헬퍼 2파일: CsvEscape, NeisHelper) -> 4단계 완료 시 211개
...
- [-] ~~`EnrollmentService.PromoteStudentsAsync`~~ — **대상과 테스트가 함께 사라졌다**(2026-08-23).
  화면에서 부를 방법이 없어 한 번도 실사용되지 않았고, 그 사이 자란 결함을 회귀 테스트가
  **현실에 없는 데이터(2학기 학적)로 통과시키고 있었다**. 실사용 경로가 없는 코드에 붙은 테스트는
  안전망이 아니라 잘못된 안심이라는 사례로 남긴다
  (GraduateAsync 는 졸업 마감 불필요 판단으로 2026-07-15 제거)
