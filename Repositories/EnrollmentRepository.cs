...
public class EnrollmentRepository : BaseRepository
{
    ...
    private void AddEnrollmentParameters(SqliteCommand cmd, Enrollment enrollment)
    {
        ...
        // 컬럼은 TEXT(yyyy-MM-dd). 모델은 DateTime? 이므로 여기서만 글자로 바꾼다 —
        // ApplyChangeAsync 도 같은 형식으로 쓴다(둘이 어긋나면 정렬·비교가 깨진다).
        cmd.Parameters.AddWithValue("@ChangeDate",
            enrollment.ChangeDate.HasValue
                ? enrollment.ChangeDate.Value.ToString("yyyy-MM-dd")
                : (object)DBNull.Value);
    ...
    private Enrollment MapEnrollment(SqliteDataReader reader, ReaderColumnCache cache)
    {
        ...
        var enrollment = new Enrollment
        {
            ...
            // 형식이 어긋난 글자가 들어 있어도(옛 데이터) 터지지 않게 TryParse 로 읽는다 —
            // 못 읽으면 "날짜를 모른다"(null) 로 둔다. 경고가 안 뜰 뿐 나머지는 멀쩡하다.
            ChangeDate = ParseChangeDate(reader, changeDateIdx),
    ...
    /// <summary>
    /// <c>ChangeDate</c> 컬럼(TEXT)을 날짜로 읽는다. NULL 이거나 못 읽으면 <c>null</c>.
    ///
    /// <para>못 읽는 글자를 만나도 던지지 않는 것이 중요하다 — 이 매퍼는 명렬표 조회가
    /// 모두 지나는 길목이라, 여기서 터지면 <b>학생 목록이 통째로 안 뜬다.</b>
    /// 날짜 하나를 모르는 것보다 그쪽이 훨씬 나쁘다.</para>
    /// </summary>
    private static DateTime? ParseChangeDate(SqliteDataReader reader, int ordinal)
    {
