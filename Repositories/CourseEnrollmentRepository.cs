...
public class CourseEnrollmentRepository : BaseRepository
{
    ...
    /// <summary>
    /// 배정은 두고 <b>강의실 지정만</b> 비운다 — 강의실을 다시 정할 때 쓴다.
    ///
    /// <para>이 표의 <c>Room</c> 은 <c>UNIQUE(EnrollmentNo, CourseNo)</c> 에 들어 있지
    /// 않다. 학생 한 명은 한 수업에 한 번 배정되고, 강의실은 거기 달린 <b>속성</b>이다.
    /// 그래서 시간표·시수·진도와 달리 <b>행을 지우지 않는다</b> — 강의실 이름이 바뀌었다고
    /// 수강생을 명단에서 빼는 것은 말이 안 된다.</para>
    ///
    /// <para>학급 공통 수업이면 이 값은 학적에서 만든 파생값(<c>"{학년}-{반}"</c>)이라
    /// 배정 화면의 [일괄 배정] 한 번으로 되돌아온다. 선택 과목·동아리는 사람이 고른
    /// 분반이라 다시 지정해야 한다 — 그 차이를 경고에 적는다.</para>
    /// </summary>
    public async Task<int> ClearRoomsByCourseAsync(int courseNo)
    {
        const string query =
            "UPDATE CourseEnrollment SET Room = '' WHERE CourseNo = @CourseNo AND IFNULL(Room, '') <> ''";
        ...
        try
        {
            ...
            LogInfo($"수강 배정 강의실 초기화: CourseNo={courseNo}, {affected}건");
            return affected;
        }
        catch (Exception ex)
        {
            LogError($"수강 배정 강의실 초기화 실패: CourseNo={courseNo}", ex);
            throw;
        }
    }
