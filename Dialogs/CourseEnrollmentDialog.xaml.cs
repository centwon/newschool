...
public sealed partial class CourseEnrollmentDialog : ContentDialog
{
    ...
    /// 후보 학생 로드.
    ///
    /// <para>동아리는 학년을 가리지 않으므로 <c>grade</c> 를 넘기지 않는다 — 0 이면
    /// <c>GetEnrollmentsAsync</c> 가 학년 조건을 아예 걸지 않아 그 해 재적자가 전부 온다.
    /// 예전에는 <c>1~3</c> 을 돌며 학년마다 한 번씩 물었다. 중·고등학교는 3학년까지라
    /// 결과는 같았지만, <b>학년 범위를 여기 적어 둘 이유가 없다</b> — 어느 학년이 있는지는
    /// 명부가 알고, 조회 한 번이면 끝난다.</para>
    /// </summary>
