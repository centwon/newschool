...
- **글자 수로도 한도를 입력할 수 있게 함**: 교육부 지침은 "500자·700자"처럼 **글자 수**로 나오는데
  설정은 바이트만 받아 매번 ×3을 암산해야 했다. 영역마다 **글자 수 칸과 바이트 칸을 나란히** 두고
  한쪽을 고치면 다른 쪽이 자동 환산된다(한글 1자 = 3바이트). 지침 숫자를 그대로 넣으면 된다.
  저장·판정 기준은 **바이트를 유지**했다 — NEIS 가 바이트로 제한하고, 영문·숫자(1바이트)가 섞이면
  글자 수와 정확히 비례하지 않기 때문이다
