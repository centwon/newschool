...
- **등록한 학생이 명부에서 사라지던 문제(가장 큰 건)** - 학년도는 사용자가 입력하는데 학기는
  `Settings.WorkSemester` 로 고정 저장했다. 2학기 중에 다음 학년도 학생을 등록하면
  `2027학년도 2학기` 학적만 생겨, 정작 2027년 3월에 학급 누가기록·학생 관리를 열면
  **한 명도 나오지 않는다**(두 화면 모두 학기로 필터링한다). -> **학기 선택**을 입력줄에 추가
  (기본값은 현재 학기)
- **번호 중복 검사가 학기를 무시**해, 1학기 3번 때문에 같은 학년도 2학기 3번 등록이 막혔다.
  (학기를 담을 지역변수를 만들어 두고 쓰지 않고 있었다) -> 학기별로 검사
