...
- `UseGoogle` - 구글 캘린더 연동
- `GoogleAutoSync` / `GoogleSyncIntervalMinutes` - 자동 동기화 사용·간격. 설정을 닫을 때 `App.ApplyGoogleSyncSettings` 가 지금 세션에 바로 반영한다
- 구글 캘린더 짝은 설정이 아니라 Calendar 테이블의 `GoogleId` 가 들고 있다 (`GoogleCalendarName` 은 40차, `GoogleCalendarID` 는 44차에 읽는 곳이 없어 제거)
...
- `StartWithWindows` - Windows 시작 시 자동 실행. **값의 정본은 레지스트리**(`HKCU\...\Run`)이고 설정은 사본이라, 화면 토글은 `Settings.IsStartWithWindowsRegistered()` 를 읽는다. [설정 초기화]는 이 등록도 해제한다
- `WindowWidth` / `WindowHeight` - 창 크기. **최대화 상태는 저장하지 않는다**(`OverlappedPresenter.State == Restored` 일 때만), 저장은 크기 변경이 멎고 500ms 뒤 한 번
- `LogLevel` - 로그 레벨

> 설정을 읽는 규칙: `SettingProperty.Reload()` 는 **저장된 값이 없거나 파싱에 실패하면 기본값으로 되돌린다.**
> [설정 초기화]와 복원이 이 함수 하나에 기대고 있고, 값 하나가 깨졌다고 앱이 시작하지 못하면 안 되기 때문이다.
...
`portable.txt` 유무로 루트를 정한다 — 있으면 포터블, 없으면 `%USERPROFILE%\NewSchool`(설치본).
표식은 **실행 파일 폴더를 먼저 보고, 없으면 그 부모까지** 본다(`Settings.FindPortableRoot`).
1.0 부터 앱 파일이 `bin\` 아래로 내려갔기 때문이다 — 부모를 보지 않으면 설치 폴더를 통째로
옮긴 포터블에서 `Data\` 를 `bin\` 안으로 밀어 넣어야 해서, 사용자 자료가 프로그램 파일
50개 사이에 섞인다. 두 모드 모두 루트 아래 배치는 같다.

| 표식 위치 | 루트 |
|---|---|
| `<루트>\portable.txt` (권장) | `<루트>\` — `bin\` 과 `Data\` 가 형제로 나란히 |
| `<루트>\bin\portable.txt` | `<루트>\bin\` — 가까운 쪽이 이겨 `Data\` 가 `bin\` 안으로 들어간다 |
...
| **학생을 완전히 지우는 기능** | **없다 — 의도한 것이다.** 학생 관리의 [명부에서 빼기]는 그 학년도 `Enrollment` 하나만 지운다. `Student`·`StudentDetail`·누가기록·학생부·사진·첨부는 남는다. 그래야 **다른 학년도 기록이 지켜진다** — `Student` 행을 지우면 그 학생의 모든 학년도 기록이 `ON DELETE CASCADE` 로 함께 날아간다. 예전에는 이 결정이 주석 처리된 호출 한 줄로만 남아 있었고(`// await studentRepo.DeleteAsync(...)`) 화면은 "학생을 삭제하시겠습니까? 되돌릴 수 없습니다" 라고 말했다(2026-09-03 문구·주석 정리). 진짜로 지우는 기능이 필요해지면 **모든 학년도 기록을 함께 지운다는 것을 사용자에게 보이는 것**부터 정해야 한다. ⚠ 마지막 학적을 빼면 그 `Student` 행은 어느 명부에도 안 나오면서 DB 에 남는다 — 빼기 전에 경고는 하지만 고아 정리는 아직 이 방향을 안 본다 | `Pages/StudentManagementPage.xaml.cs`, `DatabaseInitializer.CleanupOrphansAsync` |
